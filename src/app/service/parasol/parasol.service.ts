...
export class ParasolService {
  ...
  private baseUrl;
  private parasolsSubject: BehaviorSubject<Parasol[]> = new BehaviorSubject<
    Parasol[]
  >([]);
  private parasols$: Observable<Parasol[]> =
    this.parasolsSubject.asObservable();
  ...
  private fetchParasols(): void {
    this.getAll().then((data: Parasol[]) => {
      this.parasolsSubject.next(data);
      console.log('Parasols dans le service :', data);
    });
  }

  getAll(): Promise<Parasol[]> {
    return firstValueFrom(
      this.http.get<{ content: ParasolHttp[] }>(this.baseUrl).pipe(
        map((res) => {
  ...
  getFiles(): Promise<FileData[]> {
    return firstValueFrom(
      this.http
        .get<FileData[]>(`${this.apiUrl}/files`)
        .pipe(map((res) => res || []))
    );
  }
  getParasolsByFileId(fileId: number): Promise<Parasol[]> {
    return firstValueFrom(
      this.http
        .get<{ content: ParasolHttp[] }>(`${this.baseUrl}?ID_FILE=${fileId}`)
        .pipe(
          map((res) => {
            console.log("Réponse de l'API", res);
            return res.content.map(Parasol.mapperFromHttp);
